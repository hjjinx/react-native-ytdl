...
exports.getBasicInfo = async (id, options) => {
  ...
  let body = await fetch(url, reqOptions);
  ...
  if (jsonStr) {
    ...
    let body2 = await fetch(url, options.requestOptions);
    body2 = body2.text();
...
const gotConfig = async (id, options, additional, config, fromEmbed) => {
  ...
  let body = await fetch(url, options.requestOptions);
  body = await body.text();
...
for (let fnName of ["getBasicInfo", "getFullInfo"]) {
  ...
  exports[fnName] = async (link, options, callback) => {
    ...
    if (callback) {
      return exports[fnName](link, options).then(
